    ...
    <div class="row flex-grow-1">
      ...
      <div class="col-lg-10 d-flex flex-column pt-0 pe-0 pb-0 ps-0">
        <div class="flex-grow-1 overflow-auto">
         ...
         $pendingtraining = $conn->query("SELECT COUNT(*) AS count FROM endorsement WHERE NOT EXISTS (SELECT 1 FROM training WHERE endorsement.endorsement_id = training.endorsement_id ) AND date_endorsed IS NOT NULL AND date_endorsed != '' AND date_endorsed != '0000-00-00'")->fetch_assoc()['count'];
          $totalendorsed = $conn->query("SELECT COUNT(*) AS count FROM endorsement WHERE signed_contract = 'Yes' AND date_endorsed IS NOT NULL AND date_endorsed != '' AND date_endorsed != '0000-00-00'")->fetch_assoc()['count'];
        

          $kpiCards = [
            ['title' => 'Total Endorsed', 'count' => number_format($totalendorsed), 'icon' => 'fas fa-user-check'],
             ['title' => 'Pending Training Schedule', 'count' => number_format($pendingtraining), 'icon' => 'fas fa-database']
