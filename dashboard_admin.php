    ...
    <div class="row flex-grow-1">
      ...
      <div class="col-lg-10 d-flex flex-column pt-0 pe-0 pb-0 ps-0">
        <div class="flex-grow-1 overflow-auto">
          ...
          $totalSource = $conn->query("SELECT COUNT(*) AS count FROM source")->fetch_assoc()['count'];
          $Mysource = $conn->query("SELECT COUNT(*) AS count FROM source WHERE source_by = '$name'")->fetch_assoc()['count'];
          $New = $conn->query("SELECT COUNT(*) AS count FROM source WHERE status = 'New'")->fetch_assoc()['count'];
          $Notforrehire = $conn->query("SELECT COUNT(*) AS count FROM source WHERE status = 'Previous TA Employee not For Rehire'")->fetch_assoc()['count'];
          $Rehire = $conn->query("SELECT COUNT(*) AS count FROM source WHERE status = 'For Rehire Check'")->fetch_assoc()['count'];

          $kpiCards = [
            ['title' => 'Total Sources', 'count' => number_format($totalSource), 'icon' => 'fas fa-database'],
            ['title' => 'My Source', 'count' => number_format($Mysource), 'icon' => 'fas fa-user-check'],
            ['title' => 'New Applicants', 'count' => number_format($New), 'icon' => 'fas fa-user-plus'],
            ['title' => 'Not for Rehire', 'count' => number_format($Notforrehire), 'icon' => 'fas fa-user-times'],
            ['title' => 'For Rehire', 'count' => number_format($Rehire), 'icon' => 'fas fa-arrow-rotate-left']
          ];
