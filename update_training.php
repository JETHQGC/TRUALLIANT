...
try {
    ...
    if ($exists) {
        ...
        // Update status_date only if transitioning to 'Endorsed to Deployment'
        if ($trainee_status === 'Endorsed to Deployment' && $existing_status !== 'Endorsed to Deployment') {
            ...
            if ($info->num_rows > 0 && $info->fetch()) {
                ...
                if (!$alreadyExists) {
                    $emp_id = ''; // auto-incremented or blank
                    $emp_status = 'New';
            ...
            // Keep the original status_date if not transitioning
        ...
        // Set status_date if status is 'Endorsed to Deployment'
